...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            ...
            <!-- Bugs Table - Display all bugs in table format with status dropdowns -->
            <div class="card shadow-sm">
                <div class="card-header bg-light">
                    <h5 class="mb-0">
                        <i class="fa fa-bug"></i> Bug Details
                        <span class="badge bg-primary ms-2">{{ $bugs->count() ?? 0 }} bugs</span>
                    </h5>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($bugs as $key => $bug)
                                    <tr class="align-middle">
                                        ...
                                        <td>
                                            <!-- Status dropdown for each bug with AJAX update -->
                                            <div class="dropdown">
                                                <button class="btn btn-sm btn-outline-secondary dropdown-toggle status-btn-{{ $bug->id }}" type="button" data-bs-toggle="dropdown">
                                                    <i class="fa fa-circle text-{{ 
                                                        ...
                                                        ($bug->status === 'In Progress' ? 'warning' : 'danger')) 
                                                    }}"></i>
                                                    {{ $bug->status ?? 'Open' }}
                                                </button>
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    <li><a class="dropdown-item" href="#" onclick="updateBugStatus({{ $bug->id }}, 'Reopened'); return false;">Reopened</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="{{ route('testing.edit', $bug->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                                <i class="fa fa-pencil"></i> Edit
                                            </a>
                                        </td>
                                    </tr>

                                    <!-- Steps Modal for viewing detailed steps -->
                                    @if($bug->steps_to_reproduce)
...
function updateBugStatus(bugId, status) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("testing.updateStatus", ":id") }}'.replace(':id', bugId),
                type: 'PUT',
                data: {
                    _token: '{{ csrf_token() }}',
                    status: status
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        const statusColor = 
                            ...
                            status === 'Open' ? 'primary' : 
                            status === 'In Progress' ? 'warning' : 'danger';
                ...
                error: function(xhr) {
                    Swal.fire({
                        title: 'Error!',
                        text: 'Failed to update bug status.',
                        icon: 'error',
