...
<div class="table-responsive">
    <table class="table custom-table" id="DataTables_Table_0">
                        <thead>
                            <tr>
                                <th>Employee</th>
                                <th>Task Name</th>
    ...
    <button onclick="prevPage()" class="btn btn-sm btn-secondary">Prev</button>
    <span id="pageInfo"></span>
    <button onclick="nextPage()" class="btn btn-sm btn-secondary">Next</button>
</div>
                </div>
...
function loadTasks(filters = {}) {
   fetch(`{{ route('project.tasks', $project->projectid) }}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-CSRF-TOKEN": "{{ csrf_token() }}"
...
function renderTable() {
    ...
    let pageData = taskData.slice(start, end);
    ...
    document.getElementById("pageInfo").innerText =
        `Page ${currentPage} of ${Math.ceil(taskData.length / rowsPerPage)}`;
}
function nextPage(){
    if(currentPage * rowsPerPage < taskData.length){
        currentPage++;
        renderTable();
    }
}
