...
.hb-cat-item.active {
    ...
    border-left-color: var(--hb-orange);
}
...
/* View only label */
.hb-perms-view-only { font-size: 11px; color: #64748b; }
/* ── Page header ── */
.hb-page-header {
    background: linear-gradient(135deg, var(--hb-orange) 0%, var(--hb-orange-dark) 100%);
...
/* Empty state */
.hb-empty { text-align: center; padding: 60px 20px; color: #94a3b8; }
.hb-empty i { font-size: 40px; margin-bottom: 12px; display: block; }
</style>
...
<div class="content container-fluid" style="padding-top:16px;">
    ...
    <div class="hb-page-header d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
            ...
            <h3><i class="fas fa-plus-circle me-2"></i>Create Hierarchy Level</h3>
            <p>Define a new role level and configure its module access permissions.</p>
        </div>
    ...
    <form id="hbForm" method="POST" action="{{ route('hierarchy.store') }}" novalidate>
        @csrf
        ...
        <div class="hb-wrap">
            ...
            <div class="hb-sidebar">
                ...
                <div class="hb-sidebar-top">
                    ...
                    <input type="text" name="hierarchy_level" id="hbLevelName" class="hb-name-input @error('hierarchy_level') is-invalid @enderror"
                           placeholder="e.g. Manager, HR Admin…" value="{{ old('hierarchy_level') }}" required>
                ...
                <!-- Category navigation -->
                <nav class="hb-cat-nav" id="hbCatNav">
                    <!-- Injected by JS -->
                </nav>
            ...
            <!-- ── RIGHT CONTENT ── -->
            <div class="hb-content" id="hbContent">
...
CATEGORIES.forEach(function(cat, ci) {
    ...
    cat.modules.forEach(function(mod) {
        ...
        mod.perms.forEach(function(p) {
            ...
            const iconStr  = PERM_META[p] ? PERM_META[p].icon : 'fa-check';
            pills += `<label class="hb-pp">
                <input type="checkbox" name="modules[${mod.key}][${p}]" value="1" data-mod="${safeId}">
                <span class="hb-pp-inner"><i class="fas ${iconStr}"></i> ${labelStr}</span>
            </label>`;
        });
        permHtml = `<div class="hb-perms" id="perms-${safeId}">
            <div class="hb-perms-label">Permissions</div>
            <div class="hb-perms-row">${pills}</div>
        </div>`;

        grid += `<div class="hb-mod-card" id="card-${safeId}">
            <div class="hb-mod-header">
                ...
                <label class="hb-toggle-wrap" onclick="event.stopPropagation()">
                    <input type="checkbox" name="modules[${mod.key}][enabled]" value="1"
                           id="toggle-${safeId}" class="module-toggle" data-mod="${safeId}" data-cat="${cat.id}">
...
document.getElementById('btnSelectAll').addEventListener('click', function() {
    ...
    /* Also check all permission pills */
