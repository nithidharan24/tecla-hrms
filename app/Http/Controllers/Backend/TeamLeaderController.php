...
class TeamLeaderController extends Controller
{
    ...
    {
        $teamLeadId   = Session::get('user_id');
        $today       = now()->format('Y-m-d');

        // Team Leader's own department
        $teamLeader = DB::table('allemployees')->where('id', $teamLeadId)->first();
        $departmentId = $teamLeader->department ?? null;
        ...
        $teamStrength = DB::table('allemployees')
            ...
            ->where(function($q) use ($teamLeadId) {
                $q->where('team_lead_id', $teamLeadId)->orWhere('id', $teamLeadId);
            })
            ->count();
        ...
        $todayAttendance = DB::table('allemployees as ae')
            ...
            ->where(function($q) use ($teamLeadId) {
                $q->where('ae.team_lead_id', $teamLeadId)->orWhere('ae.id', $teamLeadId);
            })
            ->get();
        ...
        $pendingLeavesList = DB::table('employee_leaves as el')
            ...
            ->where(function($q) { $q->whereNull('el.tl_approved')->orWhere('el.tl_approved', 'pending'); })
            ->select(
        ...
        $pendingPermissionsList = Schema::hasTable('employee_permissions')
            ? DB::table('employee_permissions as ep')
                ...
                ->where(function($q) { $q->whereNull('ep.tl_approved')->orWhere('ep.tl_approved', 'pending'); })
                ->select(
        ...
        // ── TASKS ─────────────────────────────────────────────────────────────
        $teamEmployeeIds = DB::table('allemployees')
            ->where('deleted_at', 0)
            ->where(function($q) use ($teamLeadId) {
                $q->where('team_lead_id', $teamLeadId)->orWhere('id', $teamLeadId);
            })
        ...
        $taskBase = DB::table('tasks')
            ->whereIn('assigned_to', $teamEmployeeIds)
            ->where('deleted_at', 0);
        ...
        $projectTaskStatus = DB::table('tasks as t')
            ...
            ->whereIn('t.assigned_to', $teamEmployeeIds)
        ...
        $importantTasksList = DB::table('tasks as t')
            ->join('allemployees as ae', 't.assigned_to', '=', 'ae.id')
            ->leftJoin('projects as p', 't.projects', '=', 'p.projectid')
            ->whereIn('t.assigned_to', $teamEmployeeIds)
            ->where('t.deleted_at', 0)
            ->whereNotIn('t.status', ['completed'])
            ->whereIn('t.priority', ['high', 'urgent'])
            ->select('t.id', 't.task', 't.due_date', 't.priority', 't.status', 'ae.firstname', 'ae.lastname', 'p.projectname')
        ...
        return view('hrms.admin.teamleaderdashboard.index', compact(
            'departmentName',
            'departmentId',
            // Attendance
            'teamStrength',
        ...
        try {
            if ($type === 'leave') {
                ...
                foreach (['tl_approved','hr_approved','manager_approved'] as $col) {
                    if (isset($leave->$col) && $leave->$col === 'approved') $approved++;
                    elseif (isset($leave->$col) && $leave->$col === 'declined') $declined++;
                }
                ...
                $approved = 0; $declined = 0;
                foreach (['tl_approved','hr_approved','manager_approved'] as $col) {
                    if (isset($perm->$col) && $perm->$col === 'approved') $approved++;
                    elseif (isset($perm->$col) && $perm->$col === 'declined') $declined++;
                }
                $overall = $approved >= 2 ? 'approved' : ($declined >= 2 ? 'declined' : 'pending');
