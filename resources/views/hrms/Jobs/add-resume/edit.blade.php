    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('add-resume.update', $candidate->id) }}" method="POST" enctype="multipart/form-data" id="editCandidateForm">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('first_name') is-invalid @enderror"
                                           name="first_name" value="{{ old('first_name', $candidate->first_name) }}" required>
                                    @error('first_name')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('last_name') is-invalid @enderror"
                                           name="last_name" value="{{ old('last_name', $candidate->last_name) }}" required>
                                    @error('last_name')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('address') is-invalid @enderror"
                                              name="address" rows="3">{{ old('address', $candidate->address) }}</textarea>
                                    @error('address')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-control select2 @error('position_applied') is-invalid @enderror"
                                        ...
                                        @foreach($jobs as $job)
                                            <option value="{{ $job->job_title }}" 
                                                {{ old('position_applied', $candidate->position_applied) == $job->job_title ? 'selected' : '' }}>
                                                {{ $job->job_title }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('experience_years') is-invalid @enderror"
                                           name="experience_years" value="{{ old('experience_years', $candidate->experience_years) }}" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('source') is-invalid @enderror" name="source">
                                        ...
                                        @foreach(['Website', 'Referral', 'LinkedIn', 'Naukri', 'Walk-in', 'Consultancy', 'Other'] as $source)
                                            <option value="{{ $source }}" {{ old('source', $candidate->source ?? '') == $source ? 'selected' : '' }}>{{ $source }}</option>
                                        @endforeach
                                    </select>
                                    @error('source')
                                        <span class="invalid-feedback">{{ $message }}</span>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('expected_salary') is-invalid @enderror"
                                           name="expected_salary" value="{{ old('expected_salary', $candidate->expected_salary) }}" step="0.01">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" name="status" required>
                                        ...
                                        <option value="telephonic_scheduled" {{ old('status', $candidate->status) == 'telephonic_scheduled' ? 'selected' : '' }}>Telephonic Scheduled</option>
                                        <option value="telephonic_completed" {{ old('status', $candidate->status) == 'telephonic_completed' ? 'selected' : '' }}>Telephonic Completed</option>
                                        <option value="interview_scheduled" {{ old('status', $candidate->status) == 'interview_scheduled' ? 'selected' : '' }}>Interview Scheduled</option>
                                        <option value="interview_completed" {{ old('status', $candidate->status) == 'interview_completed' ? 'selected' : '' }}>Interview Completed</option>
                                        <option value="selected" {{ old('status', $candidate->status) == 'selected' ? 'selected' : '' }}>Selected</option>
                                ...
                                <div class="form-group">
                                    ...
                                    @if($candidate->resume_path)
                                        <small class="form-text text-muted">
                                            Current:
                                            @php
                                                $ext = pathinfo($candidate->resume_path, PATHINFO_EXTENSION);
                                                $safeBase = preg_replace('/[^A-Za-z0-9_\-]/', '_', trim(($candidate->first_name ?? '').'_'.($candidate->last_name ?? '')));
                                                $safeBase = trim($safeBase, '_');
                                                $computedName = ($safeBase ? $safeBase.'_Resume' : 'Resume') . ($ext ? '.'.$ext : '');
                                                $displayName = $candidate->resume_original_name ?? $computedName;
                                            @endphp
                                            <a href="{{ route('add-resume.view-resume', $candidate->id) }}" target="_blank">
                                                {{ $displayName }}
                                            </a>
                                            <span class="ms-2">|</span>
                                            <a href="{{ route('add-resume.view-resume', ['id' => $candidate->id, 'download' => 1]) }}">
                                                Download
                                            </a>
                                        </small>
                                    @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('hiring_manager_id') is-invalid @enderror" name="hiring_manager_id">
                                        ...
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ old('hiring_manager_id', $candidate->hiring_manager_id ?? '') == $employee->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('team_lead_id') is-invalid @enderror" name="team_lead_id">
                                        ...
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ old('team_lead_id', $candidate->team_lead_id ?? '') == $employee->id ? 'selected' : '' }}>
                                                {{ $employee->firstname }} {{ $employee->lastname }} {{ $employee->employeeid ? '(' . $employee->employeeid . ')' : '' }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('notes') is-invalid @enderror"
                                              name="notes" rows="4">{{ old('notes', $candidate->notes) }}</textarea>
                                    @error('notes')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="submit-section">
                            <button class="btn btn-primary submit-btn" type="submit">Update Candidate</button>
...
// jQuery approach for better validation
