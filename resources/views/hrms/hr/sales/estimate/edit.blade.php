...
<div class="content container-fluid">
    ...
    <div class="row">
        <div class="col-sm-12">
            <form id="estimate" method="POST" action="{{route('estimate.update',$estimates->estimate_id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div>
                            <input type="radio" name="client_type" id="existingClient" value="existing"
                                 onclick="toggleClientFields()" {{ $client_type == 'existing' ? 'checked' : '' }}>
                            <label for="existingClient" class="col-form-label">Existing Client</label>
                            <input type="radio" name="client_type" id="newClient" value="new"
                                 onclick="toggleClientFields()" {{ $client_type == 'new' ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6" id="ProjectTypeRow">
                        ...
                        <div class="input-block">
                            <input type="radio" name="project_type" id="existingProject" value="existing"
                                 onclick="toggleProjectFields()" {{ $existing_project_id ? 'checked' : '' }}>
                            <label for="existingProject" class="col-form-label">Existing Project</label>
                            <input type="radio" name="project_type" id="newProjectType" value="new"
                                 onclick="toggleProjectFields()" {{ $new_project ? 'checked' : '' }}>
                ...
                <div class="row">
                    <div class="col-sm-6 col-md-3" id="NewClient" style="{{ $client_type == 'existing' ? 'display: none;' : '' }}">
                        <div class="input-block mb-3">
                            <label for="nw_client" class="col-form-label">Client <span class="text-danger">*</span></label>
                            <input name="nw_client" id="nw_client" class="form-control" type="text" value="{{ $new_client }}">
                        </div>
                    </div>
                                        
                    <div class="col-sm-6 col-md-3" id="NewProject" style="{{ $existing_project_id ? 'display: none;' : '' }}">
                        <div class="input-block mb-3">
                            <label for="nw_project" class="col-form-label">Project <span class="text-danger">*</span></label>
                            <input name="nw_project" id="nw_project" class="form-control" type="text" value="{{ $new_project }}">
                        </div>
                    </div>
                                        
                    <div class="col-sm-6 col-md-3" id="ExClient" style="{{ $client_type == 'new' ? 'display: none;' : '' }}">
                        <div class="input-block mb-3">
                            ...
                            <select name="ex_client" id="ex_client" class="select" onchange="fetchProjects(this.value)">
                                ...
                                @foreach ($clients as $cs)
                                    <option value="{{ $cs->client_id }}" {{ $existing_client_id == $cs->client_id ? 'selected' : '' }}>
                                        {{ ucfirst($cs->first_name) . ' ' . ucfirst($cs->last_name) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-sm-6 col-md-3" id="ExProject" style="{{ $client_type == 'new' ? 'display: none;' : '' }}">
                        <div class="input-block mb-3">
                            ...
                            <select name="ex_project" id="ex_project" class="select">
                                ...
                                @foreach ($projects as $item)
                                    <option value=" {{$item->projectid }} " {{$item->projectid==$estimates->project ? 'selected':''}}>{{ucFirst($item->projectname)}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                                        
                    <div class="col-sm-6 col-md-3" id="ExNewProject" style="display: {{ $existing_project_id ? 'none' : ''}};">
                        <div class="input-block mb-3">
                            <label for="ex_nw_project" class="col-form-label">Project <span class="text-danger">*</span></label>
                            <input name="ex_nw_project" id="ex_nw_project" class="form-control" type="text" value="{{ $new_project }}">
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6 col-md-3">
                        <div class="input-block mb-3">
                            ...
                            <input name="email" value="{{$estimates->email}}" id="email" class="form-control" type="email">
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-3">
                        <div class="input-block mb-3">
                            <label for="mobile" class="col-form-label">Mobile <span class="text-danger">*</span></label>
                            <input name="mobile" value="{{old('mobile',$estimates->phone)}}" id="mobile" class="form-control" type="text">
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6 col-md-3">
                        <div class="input-block mb-2">
                            ...
                            <textarea name="client_address" id="cs_address" class="form-control" style="resize: none;" rows="4"  oninput="updateBillingAddress()">{{$estimates->client_address}}</textarea>
                        </div>
                        <div class="input-block mb-1">
                            <input type="checkbox" id="sameAddress" onclick="toggleAddress()" 
                                {{ trim($estimates->client_address) === trim($estimates->billing_address) ? 'checked' : '' }} />
                    ...
                    <div class="col-sm-6 col-md-3">
                        <div class="input-block mb-3">
                            ...
                            <textarea name="billing_address" id="bill_address" class="form-control" style="resize: none;" rows="4">{{$estimates->billing_address}}</textarea>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="input-block mb-3">
                            ...
                            <div class="cal-icon">
                                <input name="estdate" value="{{$estimates->estimate_date}}" id="esdate" class="form-control datetimepicker" type="text">
                            </div>
                        </div>
                        ...
                        <div class="input-block mb-3">
                            ...
                            <div class="cal-icon">
                                <input name="expdate" value="{{$estimates->expiry_date}}" class="form-control datetimepicker" id="exdate" type="text">
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-3">
                        <div class="input-block mb-3">
                            ...
                            <select name="taxes[]" id="tax-select" class="select" multiple="multiple">
                                ...
                                @php
                                    $invoiceTaxIds = explode(',', $estimates->tax);
                                @endphp
                                @foreach ($tax as $tx)
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="table-responsive">
                            @php
                            $itemIndex = count($estimate_items);
                            @endphp
                            <table class="table table-hover table-white" id="itemsTable">
                                ...
                                <tbody class="tbodyone" id="item-rows">
                                    @foreach ($estimate_items as $index => $item)
                            ...
                            <table class="table table-hover table-white">
                                <tbody id="table-body">
                                    <tr>
                                        ...
                                        <td class="text-end pe-4"><input class="form-control text-end" value="{{$estimates->total}}" id="total" name="total" readonly type="text"></td>
                                    </tr>
                                    <tr>
                                        <td colspan="5" class="text-end">Tax</td>
                                        <td class="text-end pe-4">
                                            <input class="form-control text-end" value="{{ (int) $estimates->tax_amt }}" id="tax-Amt" name="totaltaxAmt" type="hidden">
                                            <div id="taxBreakdownDisplay"></div>
                                            <div class="form-control text-end" readonly>
                                                <span id="total-tax" contenteditable="false">{{ (int) $estimates->tax_amt }}</span>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="text-end pe-4">
                                            <input class="form-control text-end" value="{{$estimates->discount !==0 ? $estimates->discount : '0'}}" min="0" max="100" id="discount" name="discount" type="number">
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="text-end tdata-width pe-4">
                                            <input class="form-control text-end" value="{{$estimates->grant_amt}}" name="grantAmt" id="grandTotal" type="text" readonly>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="input-block mb-3">
                                    ...
                                    <textarea name="other" id="others" class="form-control" style="resize: none;" rows="4">{{$estimates->other_information!=null ? $estimates->other_information : ''}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
