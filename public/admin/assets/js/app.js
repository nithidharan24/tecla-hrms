...
$(document).ready(function() {
	...
	$(document).ready(function() {
		...
		$('#searchButton').on('click', function(e) {
			...
			if (companyName) {
				table.columns(0).search(companyName);
			}
			if (clientId) {
				table.columns(1).search(clientId);
			}
			if (clientName) {
				table.columns(2).search(clientName);
			}
		...
		$('#search_Button').on('click',function (e) { 
			...
			var companySearch = $('#company_Name').val().trim().toLowerCase();
			var clientNameSearch = $('#client_Name').val().trim().toLowerCase();
			var clientIdSearch = $('#client_Id').val().trim().toLowerCase();	
			var matchesArray = [];
			var matchFound = false;

		$('.staff-grid-row .col-md-4').each(function() {
				var companyName = $(this).find('.user-name-cname').first().text().toLowerCase();
				var clientName = $(this).find('.user-name-name').last().text().toLowerCase();
				var client_Id = $(this).find('.user-name-id').last().text().toLowerCase();
				...
				if (clientNameSearch && !clientName.includes(clientNameSearch)) {
					matches = false;
				}
				if (clientIdSearch && !client_Id.includes(clientIdSearch)) {
					matches = false;
				}
				if (matches) {
					matchesArray.push($(this));
					matchFound = true;
				}
			});

			$('.staff-grid-row .col-md-4').hide();
	
			if (matchFound) {
				matchesArray.forEach(function(card) {
					card.show();
					$('.staff-grid-row').prepend(card);
				});
			} else {
				Swal.fire({
					icon: 'warning',
