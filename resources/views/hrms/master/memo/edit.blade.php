        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">Edit Memo Template</h3>
                    <a href="{{ route('memo.index') }}" class="btn btn-secondary">
                        <i class="fas fa-arrow-left"></i> Back
                    </a>
                </div>
                <form action="{{ route('memo.update', $memo->id) }}" method="POST" id="memo-form">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $memo->name) }}" required>
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('subject') is-invalid @enderror" id="subject" name="subject" value="{{ old('subject', $memo->subject ?? '') }}" placeholder="e.g. Official Memo Regarding Work Conduct" required>
                                    @error('subject')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control memo-code-editor @error('content') is-invalid @enderror" id="content" name="content" rows="18" required>{{ old('content', $memo->content) }}</textarea>
                            @error('content')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
