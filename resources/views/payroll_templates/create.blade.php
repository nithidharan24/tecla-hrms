...
<div class="content container-fluid">
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="page-title">Add Payroll Template</h3>
                <ul class="breadcrumb">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('payroll-template.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <button type="button" class="btn btn-sm btn-info float-end"
                                    onclick="loadDefaultTemplate()">Load Default Payslip Template</button>
                            <textarea class="form-control @error('content') is-invalid @enderror"
                                      id="content" name="content" rows="30" required>{{ old('content') }}</textarea>
                            @error('content')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <small class="form-text text-muted">
                                Available variables:
                                <code>@{{ $payslip->firstname }}</code>,
                                <code>@{{ $payslip->lastname }}</code>,
                                <code>@{{ $payslip->basic_salary }}</code>,
                                <code>@{{ $bankInfo }}</code>,
                                <code>@{{ $company_name }}</code>,
                                <code>@{{ $company_logo }}</code>, etc.
                            </small>
                        </div>
