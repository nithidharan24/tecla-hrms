...
<div class="mgr-dashboard">
    ...
    <div class="dashboard-body">
        ...
        <section class="section-block">
            ...
            <div class="panel" style="margin-top:20px;">
                <div class="panel-header"><div><div class="section-eyebrow">Priority</div><h3 class="section-title">High Priority & Urgent Tasks</h3></div></div>
                @if($importantTasksList->count())
                <div class="table-scroll">
                    <table class="data-table">
                        <thead><tr><th>Task</th><th>Project</th><th>Assignee</th><th>Due Date</th><th>Priority</th></tr></thead>
                        <tbody>
                            @foreach($importantTasksList as $itask)
                            <tr>
                                <td><strong>{{ $itask->task }}</strong></td>
                                <td>{{ $itask->projectname ?? '—' }}</td>
                                <td>{{ $itask->firstname }} {{ $itask->lastname }}</td>
                                <td>{{ \Carbon\Carbon::parse($itask->due_date)->format('d M Y') }}</td>
                                <td><span class="badge {{ strtolower($itask->priority) === 'urgent' ? 'badge-red' : 'badge-orange' }}">{{ ucfirst($itask->priority) }}</span></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                @else
                <div class="empty-state">No high priority or urgent tasks pending.</div>
                @endif
            </div>
        </section>
...
(function(){
    ...
    window.mgrHandleApproval=function(type,id,action){
        ...
        fetch('{{ route("teamlead.process-approval") }}',{
            method:'POST',
