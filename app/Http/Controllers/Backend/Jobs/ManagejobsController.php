    ...
    public function index(Request $request)
{
    ...
    // Get unique departments from existing jobs for filter dropdown
    $departments = DB::table('managejobs')
        ->select('department')
        ->distinct()
    ...
    $manageJobs = $query->orderBy('created_at', 'desc')->paginate($perPage);
    
    return view('hrms.Jobs.managejobs.index', compact('manageJobs', 'departments'));
}
    ...
    public function create()
    {
        ...
        return view('hrms.Jobs.managejobs.create', compact('department', 'experiences'));
    }
public function getJobTitlesByDepartment(Request $request)
{
    ...
    $jobTitles = $this->departmentJobTitles[$department] ?? [];
  ...
  public function getSkillsByJobTitle(Request $request)
{
    ...
    public function store(Request $request)
    {
        ...
        DB::table('managejobs')->insert([
            ...
            'screening_questions' => $request->screening_questions ?? null,
            'branch_id' => $branchId,
        ...
        return redirect()->route('recruitment.index')->with('success', 'Job created successfully.');
    }
    ...
    public function show($id)
    {
        ...
        if (!$job) {
            return redirect()->route('recruitment.index', ['tab' => 'job-listings'])->with('error', 'Job not found.');
        }

        $candidateQuery = DB::table('candidates')
            ->where('position_applied', $job->job_title);
        ...
        return view('hrms.Jobs.managejobs.show', compact('job', 'candidates'));
    }
    ...
    public function edit($id)
    {
        ...
        if ($job) {
            if ($job->start_date) {
                $job->start_date = Carbon::parse($job->start_date)->format('Y-m-d');
            }
            if ($job->end_date) {
                $job->end_date = Carbon::parse($job->end_date)->format('Y-m-d');
            }
        }
        
        return view('hrms.Jobs.managejobs.edit', compact('job', 'department', 'experiences'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('recruitment.index')->with('success', 'Job updated successfully.');
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('recruitment.index')->with('success', 'Job deleted successfully.');
        } catch (\Exception $e) {
            return redirect()->route('recruitment.index')->with('error', 'Failed to delete job: ' . $e->getMessage());
        }
    }
}
